...
namespace DoImportador.Services
{
    public class Integracao
    {
        ...
        public void migraProdutoMarca(List<ConnectionProperties> connectionProperties)
        {
            try
            {
                foreach (ConnectionProperties conn in connectionProperties)
                {
                    ...
                    Hashtable input;
                    insertVinculoIntegracao(dataOriginal, iConn, EnumVinculoIntegracao.PRODUTO, "EloIntegracao", "Id");
                    _form.OnSetLog($"{conn.dbNameOrigin} migrou  o EloIntegracao do Produto marca");
                    ...
                    insertVinculoIntegracao(dataOriginal, iConn, EnumVinculoIntegracao.PRODUTO, "EloIntegracaoVariant", "Id");
                    _form.OnSetLog($"{conn.dbNameOrigin} migrou  o EloIntegracaoVariant do Produto marca");
                    ...
                    _form.OnSetLog($"{conn.dbNameOrigin} migrou o Vip Comerce do Produto marca");
                }
            }
            catch (Exception ex)
            {
        ...
        public void migraPessoa(List<ConnectionProperties> connectionProperties)
        {
            try
            {
                foreach (ConnectionProperties conn in connectionProperties)
                {
                    ...
                    _form.OnSetLog($"{conn.dbNameOrigin} migrou  o EloIntegracao do cliente");
                }
            }
            catch (Exception ex)
            {
        ...
        public void migraGrupoSubGrupo(List<ConnectionProperties> connectionProperties)
        {
            foreach (ConnectionProperties conn in connectionProperties)
            {
                DOFunctions.LoadHost(conn);
                var dataOriginal = LoadData.LoadDataDb("SQLSERVER", "SELECT EloIntegracao,Id FROM produtos_grupos_subgrupos where EloIntegracao is not null AND EloIntegracao<> ''", EnumDataLake.ORIGIN);
                var iConn = new DOConn();
                iConn.ConnectionOpen("", Enum.EnumDataLake.ORIGIN);
                string query;
                Hashtable input;
                insertVinculoIntegracao(dataOriginal, iConn, EnumVinculoIntegracao.GRUPOPRODUTO, "EloIntegracao", "Id");
                _form.OnSetLog($"{conn.dbNameOrigin} migrou  o EloIntegracao do Grupo");
                dataOriginal = LoadData.LoadDataDb("SQLSERVER", "SELECT EloIntegracaoVariant,Id FROM produtos_grupos_subgrupos where EloIntegracaoVariant is not null AND EloIntegracaoVariant<> ''", EnumDataLake.ORIGIN);
                insertVinculoIntegracao(dataOriginal, iConn, EnumVinculoIntegracao.GRUPOPRODUTO, "EloIntegracaoVariant", "Id");
                _form.OnSetLog($"{conn.dbNameOrigin} migrou  o EloIntegracaoVariant do Grupo");
                dataOriginal = LoadData.LoadDataDb("SQLSERVER", "select id_vipCommerce,Id from produtos_grupos_subgrupos where id_vipCommerce is not null and id_vipCommerce<>'';", EnumDataLake.ORIGIN);
                foreach (var d in dataOriginal)
                {
                    query = "INSERT INTO vinculo_integracao (IDVinculo, IDTipoVinculo, IDIntegracao, ExternalID) VALUES (@IDVinculo, @IDTipoVinculo, @IDIntegracao, @ExternalID)";
                    input = new Hashtable();
                    input.Add("IDVinculo", d["Id"].ToString());
                    input.Add("IDTipoVinculo", 12);
                    input.Add("IDIntegracao", Enum.EnumIntegracao.VIPCOMERCE);
                    input.Add("ExternalID", d["id_vipCommerce"].ToString());
                    CrudUtils.ExecuteQuery(iConn, input, query);
                }
        ...
        public void migraProdutosUnidades(List<ConnectionProperties> connectionProperties)
        {
            foreach (ConnectionProperties conn in connectionProperties)
            {
                ...
                string query;
                Hashtable input;
                var dataOriginal = LoadData.LoadDataDb("SQLSERVER", "select id,id_vipCommerce from produtos_unidades where id_vipCommerce is not null and id_vipCommerce<>'';", EnumDataLake.ORIGIN);
                foreach (var d in dataOriginal)
                {
                    query = "INSERT INTO vinculo_integracao (IDVinculo, IDTipoVinculo, IDIntegracao, ExternalID) VALUES (@IDVinculo, @IDTipoVinculo, @IDIntegracao, @ExternalID)";
                    input = new Hashtable();
                ...
                _form.OnSetLog($"{conn.dbNameOrigin} migrou o Vip Comerce da unidade");
            }
        }
